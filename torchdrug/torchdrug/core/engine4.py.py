...
class Engine(core.Configurable):
    ...
    def train(self, num_epoch=1, batch_per_epoch=None):
        ...
        for epoch in self.meter(num_epoch):
            ...
            acc_val.append(vacc)
            wandb.log({'wandb_val_loss_epoch':vloss, 'wandb_train_loss_epoch':avg_loss_train, 'wandbmy_val_accuracy':vacc, 'my_epoch': epoch})
            if self.scheduler:
                self.scheduler.step(vloss)
            # if self.scheduler.is_better(vloss, self.scheduler.best):
            #     print(f'Scheduler triggered. New learning rate: {self.optimizer.param_groups[0]["lr"]}')
    ...
    def evaluate(self, split, log=True):
        ...
        print('Final engine_eval_name = ', eval_names)
        print('Final engine_preds = ', my_pred)
        print('Final engine_targets = ', my_target)
