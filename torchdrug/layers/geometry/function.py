...
class AlphaCarbonNode(nn.Module, core.Configurable):
    ...
    def forward(self, graph):
        ...
        try:
            ...
            # print(graph)
            # print(f'graph.num_node: { graph.num_node}')
            # print( f'graph.num_residue: { graph.num_residue}')
            # if graph.num_node != graph.num_residue
            #     print(graph)
            ...
            # This block will be executed if an AttributeError occurs
            print("AttributeError:", e)
            # Handle the error here, such as providing a default value
            atom_name = "DefaultAtomName"
            traceback.print_exc()
            return None

        except AssertionError as e:
            # Handle AssertionError here
            print("AssertionError:", e)
            # Track the file that raised the AssertionError
            traceback.print_exc()
            return None
