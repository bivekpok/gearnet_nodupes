...
# def check_parameters(model, phase):
#     print(f"Checking parameters after {phase} phase:")
#     hash_function = hashlib.sha256()

#     for name, param in model.named_parameters():
#         param_data = param.data.cpu().numpy().tobytes()
#         hash_function.update(param_data)
#         print(f"Parameter {name}: {hash_function.hexdigest()}")
...
class Engine(core.Configurable):
    ...
    def train(self, num_epoch=1, batch_per_epoch=None):
        ...
        # model.train()
        t_total_params = sum(p.numel() for p in model.parameters())
        print(f"Total number of training parameters in engine parameters: {t_total_params}")

        all_train_loss_values = [] #added lines   
        all_valid_loss_values = []
       
        train_accuracy_values = []
        val_accuracy_values = []
        all_epoch_values = []
        print(f' len( train_dataloader) {len(dataloader)}')
        print(len(self.train_set))
        # valid_dataloader = data.DataLoader(self.valid_set, self.batch_size, sampler=sampler, num_workers=self.num_worker)
        ...
        for epoch in self.meter(num_epoch):
            ...
            # gc.collect()  # Added If you are using CUDA, you can also clear the cached memory:

       
            # if self.device.type == "cuda":
            #     torch.cuda.empty_cache()
            sampler.set_epoch(epoch)
            all_epoch_values.append(epoch)
            loss_values = []#added lines
            train_metrics = []
            ...
            train_total = 0
            model.train()
            # check_parameters(model, " inside training")
            metrics = []
            start_id = 0
            ...
            for batch_id, batch in enumerate(islice(dataloader, batch_per_epoch)):
                ...
                train_correct += (predict == target).sum().item()
                
                metrics.append(metric)

                if batch_id - start_id + 1 == gradient_interval: #updates only after certain interval
                   
                    self.optimizer.step()
                    self.optimizer.zero_grad()
                    loss_values.append(loss) # added
                    ...
                    train_metrics.append(metric) # added
            ...
            train_lv = utils.stack(loss_values, dim = 0)
          
            all_train_loss_values.append(utils.mean(train_lv, dim = 0))
           
            model.eval()
            vloss, val_accuracy = self.evaluate2(model)
            all_valid_loss_values.append(vloss)
            val_accuracy_values.append(val_accuracy)
            

            if self.scheduler:
                self.scheduler.step()
        csv_base_path = '/ocean/projects/bio230029p/bpokhrel/plot_data'
        print(f' all_train_loss_values: { all_train_loss_values}')
        print(f'all_valid_loss_values: { all_valid_loss_values}')
        print(f'epoch { all_epoch_values}')

        # Ensure the directory exists; create it if it doesn't
        os.makedirs(csv_base_path, exist_ok=True)

        # Initialize variables
        file_prefix = "train_valid_info"
        file_number = 1

        while True:
            # Generate the CSV file path for the current iteration
            csv_file_path = os.path.join(csv_base_path, f"{file_prefix}_{file_number}.csv")

            # Check if the CSV file already exists
            if not os.path.exists(csv_file_path):
                # CSV file does not exist, so save the data with this filename
                with open(csv_file_path, 'w', newline='') as csvfile:
                    fieldnames = ['epoch', 'all_train_loss_values', 'all_valid_loss_values', 'train_accuracy_values', 'val_accuracy_values']
                    writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
                    writer.writeheader()


                    for epoch, t_loss, v_loss, tacc, vacc in zip(all_epoch_values, all_train_loss_values, all_valid_loss_values, train_accuracy_values, val_accuracy_values):
                        writer.writerow({'epoch': epoch, 'all_train_loss_values': t_loss.item(), 'all_valid_loss_values': v_loss, 'train_accuracy_values':tacc, 'val_accuracy_values':vacc})  # Replace your_loss_value with the actual loss value
                    break  # Exit the loop
            file_number += 1
    ...
    def evaluate(self, split, log=True):
        ...
        model.eval()
        v_total_params = sum(p.numel() for p in model.parameters())
        print(f"Total number of val parameters in engine parameters: {v_total_params}")
        ...
        for batch in dataloader:
            ...
            eval_names.append(batch['name'])
        # print('eval_names')
        # print(eval_names)
        base_path = "/ocean/projects/bio230029p/bpokhrel/names"

        file_prefix = "eval_names"
        file_number = 1
        while True:
    ...
    @torch.no_grad()
    def evaluate2(self,model, num_epoch=1, batch_per_epoch=None):
        """
        Evaluate the model.
        ...
        #     logger.warning("Evaluate on %s" % split)
        # test_set = getattr(self, "%s_set" % split)
        sampler = torch_data.DistributedSampler( self.valid_set, self.world_size, self.rank)
        dataloader = data.DataLoader(self.valid_set, self.batch_size, sampler=sampler, num_workers=self.num_worker)
        print(f'e2: len(dataloader) = {len(dataloader)}')
        ...
        # check_parameters(model, "validation")
        # total_params = sum(p.numel() for p in model.parameters())
        # print(f"Total number of e2 parameters: {total_params}")
        # preds = []
        # targets = []
        ...
        # preds = np.argmax(preds, axis = 1).cpu()
        # targets = torch.cat(targets, dim=0)
        # print(f'all_preds shape in engine {preds.shape}')
        # print(f'all_target shape in engine {targets.shape}')

        # # Assuming predict_and_target returns raw logits for preds
        ...
        for batch_id, batch in enumerate(dataloader):
            ...
            val_correct += (predict == target).sum().item()
            # print('inside eval2')
            # print(val_correct)
            # print(val_total)
            # print(f'2 _loss {loss_values}')
        ...
        print(f'loss_values length {len(loss_values)}')
        print(f' len(dataloader) = {len(dataloader)}')
        
        # lv = utils.stack(loss_values, dim = 0)
        # final_mean_loss = utils.mean(lv, dim = 0)
        # print(f'final_mean loss : {final_mean_loss.item()}')
